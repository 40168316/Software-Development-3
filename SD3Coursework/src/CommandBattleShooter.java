...
//Public class for the BattleShooter which implements options which is part of the command pattern. This can move the ship or undo the ships moves.
...
public class CommandBattleShooter extends EnemyShip implements Options {
	// Set the variables
	...
	public void move() {
		...
		System.out.println("BattleShooter move successful - " + x + " " + y);
		GUI.shootershipposx = x;
		GUI.shootershipposy = y;
		GUI.movesshooter = moves;
		loops++;
	}

	// Method which allows for the ship to return to the previous tile
	public void previousmove() {
		// Num-- is a variable which goes back one space in the movetile array list, where all the tiles are stored
		num--;
		...
		GUI.shootershipposx = x;
		GUI.shootershipposy = y;
		System.out.println("BattleShooter - Undo Move " + x +" " + y);
	}
	
	// Method which allows for the factory pattern to set the type of ship
	public CommandBattleShooter() {
		setType(" BattleShooter.");
	}
}
